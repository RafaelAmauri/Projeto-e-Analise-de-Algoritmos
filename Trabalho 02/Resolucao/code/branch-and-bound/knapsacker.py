...
class Knapsacker:
  ...
  def grow(self, node, next_index):
    ...
    if node.positive_child_growable():
      if next_item and node.cost() + next_item.cost <= self.capacity:
        upper_bound = node.value() + next_item.value + self.upper_bound_beyond(next_index, capacity=self.capacity - node.cost() - next_item.cost)
  ...
  def node_with_highest_value(self):
    return reduce(lambda provisional, candidate: provisional if provisional.value() >= candidate.value() else candidate, self.candidates)
